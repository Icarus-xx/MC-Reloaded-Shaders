...
//    float lum = (pixcol.r+pixcol.g+pixcol.b)/3.;
    ...
    {
        //Noise
        float noise =  (fract(sin(dot(TexCoord0.st ,vec2(12.0,78.0)+float(worldTime))) * 43758.0));
        //Nightvision
        ...
        vec4 vigfin_l = vec4(smoothstep(lensRadius.x, lensRadius.y, dist_nv_left));
        vec4 vigfin_r = vec4(smoothstep(lensRadius.x, lensRadius.y, dist_nv_right));
        baseColor = (vec4(gray * nvcol, texcolor.a) + noise*noiseamount) * vec4(mix(vigfin_l.rgb, vigfin_r.rgb, 0.5), 1.0);
    }
#endif
