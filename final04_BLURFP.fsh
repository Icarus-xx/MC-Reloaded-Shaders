...
main (void)
{
    ...
    sum +=
        texture2D (sampler0,
                   vec2 (TexCoord0.x - 4.0 * blurSize, TexCoord0.y)) * 0.05;
    sum +=
        texture2D (sampler0,
                   vec2 (TexCoord0.x - 3.0 * blurSize, TexCoord0.y)) * 0.09;
    sum +=
        texture2D (sampler0,
                   vec2 (TexCoord0.x - 2.0 * blurSize, TexCoord0.y)) * 0.12;
    sum +=
        texture2D (sampler0,
                   vec2 (TexCoord0.x - blurSize, TexCoord0.y)) * 0.15;
