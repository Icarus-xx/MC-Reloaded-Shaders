    ...
    vec4 baseColor;
        ...
        if (distance <= MAX_DISTANCE){
            ...
            if (tSize > 16 ) {
                ...
                multTest = clamp((pow(( texture2D(sampler1, coord.st).a + 1.2) * 0.6, 3) / ( 0.01 + PARALLAX_OCCLUSION_SHADOW_MULT)), 0.02, 1.0);
                #endif
            }
        ...
        vec3 bump =  normalize(texture2D(sampler1, coord.st).xyz*2.0-1.0);
            ...
            tSize = textureSize(envMap,0).x;
                ...
                if (tSize > 16) {
                    ...
                    baseColor = mix(baseColor, reflColor, vertColor.r * REFL_MULTIPLIER * R * texture2D(envMap, coord.st).a);
                }
        #endif
            ...
            if (tSize > 16) {
                ...
                float intensity = 1.0 - min(1.3*distance / heldLight.magnitude, 1.0);
                intensity = intensity;
                baseColor = mix(baseColor, texture2D(sampler0, coord.st) * heldLight.specular * clamp( vertColor + intensity * dot(V, bump) , 0.0, 1.0),clamp( intensity * dot(V, bump) , 0.0, 1.0));
                ...
                if (specular.rgb != vec3(0.0,0.0,0.0)) {
                    ...
                    N = normalize( gl_NormalMatrix * normal );
                        if (useCelestialSpecularMapping > 0.5 ) {
                            ...
                            float s = pow(max(0.0, dot(reflect( L, bump),V)), shininess);
                            baseColor += max(specular * s * shininess* specMultiplier, 0.0);
                        }
                        float s = pow(max(0.0, dot(reflect( -V, bump),V)), shininess);
                        baseColor += max(intensity * specular * shininess * s * heldLight.specular, 0.0);
