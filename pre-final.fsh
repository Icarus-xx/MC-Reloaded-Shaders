...
float
expDepth2linearDepth (vec2 coord)
{
...
getAA ()
{
    ...
    if (abs (depth - expDepth2linearDepth (TexCoord0.xy + vec2 (offsetx, 0)))
        > depthThreshold
        || abs (depth -
                expDepth2linearDepth (TexCoord0.xy + vec2 (-offsetx, 0))) >
        depthThreshold
        || abs (depth -
                expDepth2linearDepth (TexCoord0.xy + vec2 (0, offsety))) >
        depthThreshold
        || abs (depth -
                expDepth2linearDepth (TexCoord0.xy + vec2 (0, -offsety))) >
        depthThreshold)
    {
        newColor +=
            texture2D (sampler0, TexCoord0.st + vec2 (-offsetx, offsety));
