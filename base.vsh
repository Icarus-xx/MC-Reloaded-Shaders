...
// Reserved for later usage, should stop most "weather mayhem" from occuring in snow lands
 #ifdef WEATHER_MAYHEM
	if ( biomeType == 7 || biomeType == 10 || biomeType ==11 ) {
		rT = 0;
		wR = 0;
		wT = 0;
	}
...
//        float t = mod(float(worldTime), 200.0)/200.0;
        ...
        if (floor((16.0*gl_MultiTexCoord0.t)+0.5) <= floor(16.0*gl_MultiTexCoord0.t)) {
            position.x += ((sin(PI2*(2.0*pos.x + pos.y - 3.0*t)) + 0.6) *  (1.0 + ((1.0+wT*4)*rT)))/20.0;
        }
    }
#endif
...
// Using Water ID yields very strange result, so I'd rather exclude pistons ( 29, 33, 34 ) as well as all "living entities ( -1 ).
	...
	if (((tex >= 204 && tex <= 207) || (tex >= 221 && tex <= 223) || tex == 125) && (mc_Entity.x != 34.0 && mc_Entity.x != 29.0 && mc_Entity.x != 33.0 && mc_Entity.x != -1 && renderType == 0)) {
...
#ifdef WAVING_LAVA
  if ( ((tex >= 236 && tex <= 239) || (tex >= 253 && tex <= 255)) && mc_Entity.x != -1.0) {
//       float t1 = float(mod(worldTime, 1000))/400.0;
		t /= 700.0;
        vec2 pos = position.xz/16.0;
            position.y += (cos((PI*2.0)*(2.0 * (pos.x + pos.y) + PI * t)) + 0.2)/WAVE_PITCH;
    }
#endif

#ifdef WAVING_LEAVES
if ((tex == 52 || tex == 53 || tex == 132 || tex == 133)&& renderType == 1)  {
//        float t = mod(float(worldTime), 1000.0)/1000.0;
...
/*			position.x -= (sin(PI2*(2.0*pos.x + pos.y - 3.0*t))*float(wRTransition+200)/200.0 + 0.6) / 24.0;
            ...
            position.x -= ((sin(PI2*(2.0*pos.x + pos.y - 3.0*t)) + 0.6) * (1.0 + ((3.0 + (wT*6.0))*rT))) / 24.0;
            position.y -= ((sin(PI2*(3.0*pos.x + pos.y - 4.0*t)) + 1.2) * (1.0 + ((1.0+wT)*rT))) / 32.0;
...
//		float t = float(mod(worldTime, 300))/300.0;
        ...
        if (round(16.0*gl_MultiTexCoord0.t) <= floor(16.0*gl_MultiTexCoord0.t)) {
            position.x -= ((sin(PI2*(2.0*pos.x + pos.y - 3.0*t)) + 0.6) *  (1.0 + ((1.0+wT)*rT)))/12.0;
        }
    }
#endif

#ifdef SHORT_GRASS
    if (((87 < tex && tex < 93) || (186 < tex && tex < 192)) && renderType == 1) {
//		float t = float(mod(worldTime, 200))/200.0;
		t /= 200.0 - ( wR * ( 50.0 + ( wT * 100.0 )));
        vec2 pos = position.xz/16.0;
        if (round(16.0*gl_MultiTexCoord0.t) <= floor(16.0*gl_MultiTexCoord0.t)) {
            position.x -= ((sin(PI2*(2.0*pos.x + pos.y - 3.0*t)) + 0.6) *  (1.0 + ((1.0+wT)*rT)))/32.0;
        }
    }
#endif

#ifdef WAVING_PLANTS
if ((tex == 12 || tex == 13 || tex == 15 || tex == 28 || tex == 29 || tex == 39 || tex == 56 || tex == 63 || tex == 79 || tex == 126 || tex == 157 || tex == 158 || tex == 173 || tex == 174) && renderType == 1)  {
//        float t = mod(float(worldTime), 500.0)/500.0;
		t /= 500.0 - ( wR * ( 150.0 + ( wT * 250.0 )));
        vec2 pos = position.xz/16.0;
        if ( floor((16.0*gl_MultiTexCoord0.t)+0.5) <= floor(16.0*gl_MultiTexCoord0.t) ) {
            position.x -= ((sin(PI2*(3.0*pos.x + pos.y - 3.0*t)) + 0.6) *  (1.0 + ((1.0+wT)*rT)))/8.0;
        }
    }
#endif
...
//        float t = mod(float(worldTime), 500.0)/500.0;
        ...
        vec2 pos = position.xz/16.0;
        if (round(16.0*gl_MultiTexCoord0.t) <= floor(16.0*gl_MultiTexCoord0.t)) {
            position.x -= ((sin(PI2*(2.0*pos.x + pos.y - 3.0*t)) + 0.6) *  (1.0 + ((3.0+wT*4.0)*rT))) / 24.0;
            position.z -= (sin(PI2*(1.0*pos.x + pos.y - 1.5*t)) + 0.3) / 8.0;
		}
    }
#endif   
...
//if (tex == 14) {
	...
	if ((gl_Normal.x >= 1.0) || (gl_Normal.x < 0.0 )) {
		position.x -= (sin(PI2*(2.0*pos.x + pos.y - 4.0*t)) + 0.1) / 12.0;
	} else if ((gl_Normal.z >= 1.0) || (gl_Normal.z < 0.0)){
            position.z -= (sin(PI2*(2.0*pos.z + pos.y - 4.0*t)) + 0.1) / 12.0;
	}
	position.y -= (sin(PI2*(3.0*(pos.x+pos.z) + pos.y - 4.0*t)) + 1.2) / 32.0;
...
*/
   ...
   vertColor = gl_Color;
 ...
 #ifdef SHADOW_FIX
	int squareTex = 0;
 	if (textureSize(sampler0,0).x == textureSize(sampler0,0).y) squareTex = 1;
	if ((mc_Entity.x == -1 && gl_Color.a == 1.0) && !(gl_Color.r != gl_Color.g || gl_Color.g != gl_Color.b) && squareTex ==0 )  {				// works ok for entities that calls a non square texture - quick n dirty solution
	...
	} else if(((mc_Entity.x == -1 && gl_Color.a == 1.0) && squareTex ==1) && !(tex >= 106 && tex <= 110) )  {			// I hate pistons. Why won't they just work ? Looks like there is too much to ithem for now
//			vec3 normalFix = gl_NormalMatrix * gl_Normal;
  ...
  */
   ...
   vec3 tempVec = (gl_ModelViewMatrix * gl_Vertex).xyz;
   viewVector = normalize(tbnMatrix * tempVec);
   ...
   specMultiplier *= clamp(abs(float(worldTime) / 500.0 - 46.0), 0.0, 1.0) * clamp(abs(float(worldTime) / 500.0 - 24.5), 0.0, 1.0) * vertColor.r;
...
if (mc_Entity.a != -1) {
            ...
            else if (mc_Entity.a == 4.0 )  { gl_Position.z = 0.0; vertColor = vec4(0.5, 0.5, 1.0, 1.0); }
            else if (mc_Entity.a == 5.0 ) { gl_Position.z = 0.0; vertColor = vec4(1.0, 0.0, 0.0, 1.0); }
            else if (mc_Entity.a == 6.0 ) { gl_Position.z = 0.0; vertColor = vec4(1.0, 0.5, 0.0, 1.0); }
            else if (mc_Entity.a == 7.0 ) { gl_Position.z = 0.0; vertColor = vec4(1.0, 0.0, 0.5, 1.0); }
            else if (mc_Entity.a == 8.0 ) { gl_Position.z = 0.0; vertColor = vec4(1.0, 0.5, 0.5, 1.0); }
            else if (mc_Entity.a == 9.0 ) { gl_Position.z = 0.0; vertColor = vec4(0.0, 1.0, 0.0, 1.0); }
            else if (mc_Entity.a == 10.0 ) { gl_Position.z = 0.0; vertColor = vec4(0.5, 1.0, 0.0, 1.0); }
...
*/
	...
	if (renderType != 0 || mc_Entity.x == 90){							// 90 is the portal block ID, we want to carry this info to the frag
