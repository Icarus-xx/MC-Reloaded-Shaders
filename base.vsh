...
// Reserved for later usage, should stop most "weather mayhem" from occuring in snow lands
 #ifdef WEATHER_MAYHEM
    if ( biomeType >= 7 && biomeType < 8) {// || biomeType == 10 || biomeType ==11 ) {
        rT = 0.0;
        wR = 0.0;
        wT = 0.0;
    }
...
if (biomeType ==12) {
   ...
   useCelestialSpecularMapping = 1.0;
    ...
    specMultiplier *= clamp(abs(float(worldTime) / 500.0 - 46.0), 0.0, 1.0) * clamp(abs(float(worldTime) / 500.0 - 24.5), 0.0, 1.0) * clamp((vertColor.r - 0.5) * 2.0, 0.0, 1.0);
