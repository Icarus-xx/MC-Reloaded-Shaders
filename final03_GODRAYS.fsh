...
/*    if (worldTime > 13300 && worldTime < 22000) {            // kept for future tweaks
    ...
    if (celestPosition.z < 0 && depthGR.x < threshold && depthGR.y < 0.0)   //(worldTime < 14000 || worldTime > 22000) &&
    {
        ...
        vec2 delta =
            (texCoord - lightPos) * timeFlies ().s / float (GR_SAMPLES);
        ...
        for (int i = 0; i < GR_SAMPLES; i++)
        {
            ...
            if (getDepthGR (texCoord).x > threshold)
                sample = texture2D (sampler0, texCoord).rgb;
            sample *= decay;
            if (distance (texCoord, lightPos) > 0.05)
                sample *= 0.2;
            color += sample;
            decay *= GR_DECAY;
        }
        baseColor =
            mix (baseColor, vec4 (1.0) * getAmbient (),
                 timeFlies ().t * vec4 (color, 1.0));
    }
#endif
