...
float
getBrightness (vec4 color)
{
    float Brightness =
        color[0] * 0.299f + color[1] * 0.587f + color[2] * 0.114f;
    return Brightness;
}
...
//        float depth = getDepth( TexCoord0.st );
    ...
    {                           // sunrise! Bloom attenuation operations in order to safeguard our poor eyes.
    ...
    {                           // night time ! Bloom augmentation operations to get some free candies.
...
#ifdef FPS_GAIN
    for (int i = 1; i <= B_SAMPLES; i++)
    {                           // This loop will sample 4 pixels per iteration as opposed to 8 for the "higher rig" loop.

        if (BLUR_CROSS_TYPE >= 1)
        {
            lumContrib =
                pow (getBrightness
                     (texture2D (stepSampler4, texcoord.st + vec2 (pw, ph))),
                     pwr);
            lumContrib +=
                pow (getBrightness
                     (texture2D (stepSampler4, texcoord.st + vec2 (-pw, ph))),
            ...
            lumContrib +=
                pow (getBrightness
                     (texture2D
                      (stepSampler4, texcoord.st + vec2 (-pw, -ph))), pwr);
            lumContrib +=
                pow (getBrightness
                     (texture2D (stepSampler4, texcoord.st + vec2 (pw, -ph))),
                     pwr);
        ...
        else
        {
            lumContrib =
                pow (getBrightness
                     (texture2D
                      (stepSampler4, texcoord.st + vec2 (0.0, ph * 1.5))),
                     pwr);
            lumContrib +=
                pow (getBrightness
                     (texture2D
                      (stepSampler4, texcoord.st + vec2 (pw * -1.5, 0.0))),
                     pwr);
            lumContrib +=
                pow (getBrightness
                     (texture2D
...
#else
    for (int i = 1; i <= B_SAMPLES; i++)
    {                           // This loop will sample 8 pixels per iterations around the
        ...
        lumContrib =
            pow (getBrightness
                 (texture2D
                  (stepSampler4, texcoord.st + vec2 (0.0, ph * 1.5))), pwr);
        lumContrib +=
            pow (getBrightness
                 (texture2D
                  (stepSampler4, texcoord.st + vec2 (pw * -1.5, 0.0))), pwr);
        lumContrib +=
            pow (getBrightness
                 (texture2D
                  (stepSampler4, texcoord.st + vec2 (0.0, ph * -1.5))), pwr);
...
main ()
{
    ...
    baseColor =
        (1.0 -
         ((1.0 - baseColor) * (1.0 - ((blurColor * bloom) + (bloom * 0.1)))));
