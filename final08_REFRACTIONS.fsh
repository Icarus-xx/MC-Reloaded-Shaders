...
getMaskFromDepth (vec2 coord)
{
    ...
    if (coord.x - 0.05 < 0.0)
        coord.x = coord.x + 0.05;   // should not sample outside the screen as there are no infos
    if (coord.x + 0.05 > 1.0)
        coord.x = coord.x - 0.05;   // Telling the shader to go back on the tex
    if (coord.y - 0.1 < 0.0)
        coord.y = coord.y + 0.1;
    if (coord.y + 0.1 > 1.0)
        coord.y = coord.y - 0.1;
    ...
    if (biomeType != 12)
    {
        if (depth < 1.0)
        {
            ...
            float dComp2 =
                texture2D (cleanDepth, coord + vec2 (-0.05, 0.1)).x;
...
getInWater ()
{
    ...
    if (inWater == 1)
    {
        ...
        offset =
            vec3 (sin (PI2 * coord.s + t) * cos (PI2 * (coord.t + t)) *
                  cos (PI2 * t) / N,
                  -cos (PI2 * (coord.s + t)) * sin (2.0 * PI2 *
                                                    (coord.t + t)) / N, 0);
